...
void DMA7::RunChannel(Channel& channel) {
  // TODO: what happens if source control is set to reload?
  ...
  if (channel.size == Channel::Size::Word) {
    while (channel.latch.length-- != 0) {
      memory->WriteWord(channel.latch.dst, memory->ReadWord(channel.latch.src, Bus::System), Bus::System);
      channel.latch.dst += dst_offset;
      channel.latch.src += src_offset;
    }
  } else {
    while (channel.latch.length-- != 0) {
      memory->WriteHalf(channel.latch.dst, memory->ReadHalf(channel.latch.src, Bus::System), Bus::System);
      channel.latch.dst += dst_offset;
      channel.latch.src += src_offset;
    }
  }
