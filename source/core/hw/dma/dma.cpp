  ...
  while (channel.latch.length != 0) {
    ...
    if (size == Channel::Half) {
      if (likely(channel.latch.src_addr >= 0x02000000)) {
        auto value = memory->ReadHalf(channel.latch.src_addr, Bus::Data);
        latch = (value << 16) | value;
      } else {
        //memory->Idle();
      }
      memory->WriteHalf(channel.latch.dst_addr, latch);
    } else {
      if (likely(channel.latch.src_addr >= 0x02000000)) {
        latch = memory->ReadWord(channel.latch.src_addr, Bus::Data);
      } else {
        //memory->Idle();
      }
      memory->WriteWord(channel.latch.dst_addr, latch);
    }
